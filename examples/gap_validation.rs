...
use eventix::{Calendar, Event, Recurrence, gap_validation, timezone};
use chrono::Duration;
...
fn main() -> anyhow::Result<()> {
    ...
    for (i, gap) in gaps.iter().enumerate() {
        ...
        println!("    Duration: {} minutes ({:.1} hours)",
    ...
    let one_hour_slots = gap_validation::find_available_slots(
        &cal,
        work_start,
        work_end,
        Duration::hours(1)
    ...
    if one_hour_slots.is_empty() {
        ...
        for (i, slot) in one_hour_slots.iter().enumerate() {
            println!("  Option {}: {} - {} ({:.1} hours available)",
                i + 1,
    ...
    let recurring_density = gap_validation::calculate_density(
        &recurring_cal,
        month_start,
        month_end
    )?;
    ...
    println!("  Total Busy Time: {:.1} hours", 
        recurring_density.busy_duration.num_minutes() as f64 / 60.0
    );
    println!("  Average per week: {:.1} hours",
